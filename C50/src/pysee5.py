...
def sim_adders():
    ...
    for adder in adders:
        ...
        classify = classify.replace('if ( Dv <= T->Forks )', 'if ( leq(Dv, T->Forks, '+ adder +', 4) )')
        # teste de subconjunto discreto
        classify = classify.replace('if ( Dv <= MaxAttVal[T->Tested] )', 'if ( leq(Dv,  MaxAttVal[T->Tested], '+ adder +', 4) )')
        ...
        with open('classify.c', 'r') as f:
            classify = f.read()
        classify = classify.replace('if ( leq(Dv, T->Forks, '+ adder +', 4) )', 'if ( Dv <= T->Forks )')
        classify = classify.replace('if ( leq(Dv,  MaxAttVal[T->Tested], '+ adder +', 4) )', 'if ( Dv <= MaxAttVal[T->Tested] )')
        with open('classify.c', 'w') as f:
            f.seek(0)
            f.write(classify)
...
def sim_dedicated():
    ...
    for comp in comparators:
        ...
        with open('classify.c', 'r') as f:
            classify = f.read()
        # teste de atributo discreto
        classify = classify.replace('if ( Dv <= T->Forks )', 'if ( '+comp+'(Dv, T->Forks) )')
        # teste de subconjunto discreto
        classify = classify.replace('if ( Dv <= MaxAttVal[T->Tested] )', 'if ( '+comp+'(Dv,  MaxAttVal[T->Tested]) )')
        with open('classify.c', 'w') as f:
            f.seek(0)
            f.write(classify)
        os.system('make')
        ...
        with open('classify.c', 'r') as f:
            classify = f.read()
        classify = classify.replace('if ( '+comp+'(Dv, T->Forks) )', 'if ( Dv <= T->Forks )')
        classify = classify.replace('if ( '+comp+'(Dv,  MaxAttVal[T->Tested]) )', 'if ( Dv <= MaxAttVal[T->Tested] )')
        with open('classify.c', 'w') as f:
            f.seek(0)
            f.write(classify)
